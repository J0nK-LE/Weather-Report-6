...
let citiesHistory = JSON.parse(localStorage.getItem("savedCities")) || [];
...
submitBtn.addEventListener("click", function () {
  if (inputCity.value === "") {
    ...
    if  (!citiesHistory.includes(cityName)) {
      citiesHistory.push(cityName);
      localStorage.setItem("savedCities", JSON.stringify(citiesHistory))
      renderHistory();
    }
    getWeather(cityName);
...
function renderHistory() {
   searchHistory.innerHTML = ""
    ...
    citiesHistory.forEach((cityHistory) => {
      console.log('CITYhistory',  cityHistory)
      console.log('CITIEShistory',  citiesHistory)
      
      button = document.createElement("button");
      button.classList.add("historyBtn");
  
      button.innerHTML = cityHistory;
  ...
  historyBtns.forEach((historyBtn) =>
    historyBtn.addEventListener("click", function () {
      let cityName = historyBtn.innerText;
      console.log('button city', cityName);
      getWeather(cityName);
    })
  );
}
...
function getWeather(cityName) {
  fetch(
    ...
    .then(function (latLonData) {
      return fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${latLonData.lat}&lon=${latLonData.lon}&appid=${apiKey}&units=imperial`
      )
        .then(function (response) {
          return response.json();
        })
  ...
  fetch(
    ...
    .then(function (latLonData) {
      return fetch(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${latLonData.lat}&lon=${latLonData.lon}&appid=${apiKey}&units=imperial`
      )
        .then(function (response) {
          return response.json();
        })
